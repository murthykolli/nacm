	...
	<script type="text/javascript">
 	$(document).ready(function() { var priceReg = /^[0-9\.]+$/;
 		$("#contRegistryButtonId").click(function () { var result = true; 		    
	 		if(priceReg.test($("#guestFundAmountId").val())){ $("#guestFundAmountId").css("border-color","black"); var guetFTCotributedNeedPrice = "<c:out value="${guestFTCotributedRemainingPrice}"/>"; var guestFTCotributedNeedPrice = guetFTCotributedNeedPrice.replace(",",""); var ftCotributedNeedPriceDb = parseFloat(guestFTCotributedNeedPrice); var ftCotributedPriceDb = parseFloat($("#guestFundAmountId").val());	 		
	 		if(ftCotributedPriceDb > ftCotributedNeedPriceDb) { alert("Please make sure the amount you wish to gift is equivalent or less than the remaining balance of funds requested."); result=false; } } else{ $("#guestFundAmountId").css("border-color","red"); result=false; } return result; }); 		
 	 	...
 	 	var regStillNeedAmount1 = "<c:out value="${guestFTCotributedRemainingPrice}"/>"; var regiStillNeedAmount = parseInt(regStillNeedAmount1);
 	 	if(regiStillNeedAmount > 0) { $("#regStillNeedAmountId").show(); } else { $("#regStillNeedAmountId").hide(); } 	 	
 	 	var words = $('#ftontributedNewNameTextId').text().split(' '); var html = '';
 	 	$.each(words, function() { html += '<span class="first_letter_dsp">'+this.substring(0,1)+'</span>'+this.substring(1) + ' '; }); $('#ftontributedNewNameTextId').html(html);
 	});
		        ...
		        <div align="left">		        
				    <div class="row">
						...
						<div class="col-md-12 col-sm-12">
							...
							<div><img src="${guestFTCotributedProductImage}" class="reg_cb_contributed_image_dsp"></div>
							</div>							
						</div>
						
				        <div class="col-md-12 col-sm-12">
				        <div align="center" style="margin-top: 20px;" id="ftontributedNewNameTextId" class="reg_cb_contributed_name_dsp">${guestFTCotributedProductName}</div>			
				        </div>
			        ...
			        <div class="row top_adjustment_60">
						<div class="col-md-12 col-sm-12">
							<div align="center" class="regi_guests_fund_rstest_dsp"><p>CONTRIBUTE ANY AMOUNT TOWARDS THE ${guestFTCotributedProductName}:</p></div>
						</div>
						
						<form action="guestFinTouchesContributedDataUpdate" method="POST">
					    ...
					    <div class="reg_guest_st_need_amt_text" id="regStillNeedAmountId" style="display: none;">STILL NEEDS: $${guestFTCotributedRemainingPrice}</div>
					    </div>
				        ...
				        <input type="hidden" name="guestFinishingTouchesContProductId" value="${guestFTCotributedProductSavingId}"> <input type="hidden" name="guestFTCotributedProductName" value="${guestFTCotributedProductName}"> <input type="hidden" name="guestFTCotributedProductImage" value="${guestFTCotributedProductImage}"> <input type="hidden" name="guestFTCotributedNeedPrice" value="${guestFTCotributedRemainingPrice}">
				        </form>				        				        	                
			        </div>
